...
ggplot( aes(x = Melt ,y = df_1clean$X.ktCO2e. / 1000, fill=Color)) + 
  ...
  annotate("segment", x = 0.5, xend = 0.5, y = -115, yend = 0,size=1)+
  annotate("segment", x = 3.5, xend = 3.5, y = -90, yend = 0)+
  annotate("segment", x = 5.5, xend = 5.5, y = -115, yend = 0,size=1)+
  annotate("segment", x =8.5, xend = 8.5, y = -90, yend = 0)+
  annotate("segment", x = 10.5, xend = 10.5, y = -115, yend = 0,size=1)+
  annotate("segment", x =13.5, xend = 13.5, y = -90, yend = 0)+
  annotate("segment", x = 15.5, xend = 15.5, y = -115, yend = 0,size=1)+
  annotate("segment", x = 0.5, xend = 15.5, y = -115, yend = -115)+
  annotate("segment", x = 0.5, xend = 15.5, y = -90, yend = -90)+
  annotate(geom = "text", x = seq_len(nrow(df))-0.1, y = (df_1clean$X.ktCO2e./1000) + 10, 
           label = as.integer(df$Change.in.Emission*100), size = 5) +
  annotate(geom = "text", x = seq_len(nrow(df))+0.3,y = (df_1clean$X.ktCO2e./1000) + 10, 
